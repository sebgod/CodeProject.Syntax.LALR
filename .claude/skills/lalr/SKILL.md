...
2. **More example grammars.** TOML config, C declaration syntax (the famous "Lexer Hack"), an arithmetic-with-unary-ops upgrade for `TestProject`. Each ships under `examples/`.
3. **Async-per-token at the parser/iterator boundary.** The lexer's inner loop is sync and the only `await` per byte-buffer is `PipeReader.ReadAsync`. The remaining async-per-token cost lives in `IAsyncIterator<Item>` callers and the parser loop. A pure-sync, `ref struct`-style parser is possible; not a priority.
