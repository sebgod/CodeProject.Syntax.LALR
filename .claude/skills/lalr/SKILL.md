...
1. **Phase 5 (compiler-compiler) — slice 5+.** Slices 1–4 landed (`ParserTableBuilder` extracted, linked into the netstandard2.0 generator, `TablesEmitter` produces `<Name>.Tables.g.cs` with a pre-baked `Definition` + `ParseTable` + `BuildParser()` per YAML grammar; `VisitorEmitter` adds a `BuildParser<T>(IVisitor<T>)` overload that splices visitor rewriters into the pre-baked grammar and reuses the pre-baked table; `LALR0004` surfaces S/R + R/R conflicts as Roslyn diagnostics at build time). Remaining slices:
   - **Slice 5 — migrate `Bootstrap.Stage1` / `Examples.Json` / `Examples.Latex` / `Examples.LatexConsole` to the pre-baked path** (drop their runtime `SchemaCompiler.Compile` calls in favour of the generated `BuildParser(visitor)`; the lexer half still routes through `Build(visitor).Lexer` until slice 6). `lalr-tui` stays runtime-build because its purpose is editing arbitrary YAML live.
