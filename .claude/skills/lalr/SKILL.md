...
1. **Phase 5 (compiler-compiler) — slice 4+.** Slices 1–3 landed (`ParserTableBuilder` extracted, linked into the netstandard2.0 generator, and `TablesEmitter` now produces `<Name>.Tables.g.cs` with a pre-baked `Definition` + `ParseTable` + `BuildParser()` per YAML grammar; `LALR0004` surfaces S/R + R/R conflicts as Roslyn diagnostics at build time). Remaining slices:
   - **Slice 4 — visitor-aware `BuildParser<T>(IVisitor<T>)`.** `BuildParser()` today is action-free; productions with an `action:` fall back to `Reduction` at parse time. The visitor-aware overload should walk `Definition`'s precedence groups, swap in rewriter-bearing `Production`s for the action-named slots (using `BuildActions(visitor)`), and pass the rewritten Grammar + the same pre-baked ParseTable to the new `Parser(Grammar, ParseTable)` ctor. Emit alongside the existing visitor-aware `Build<T>(IVisitor<T>)` in `<Name>.Visitor.g.cs`.
   - **Slice 5 — migrate `Bootstrap.Stage1` / `Examples.Json` / `Examples.Latex` / `Examples.LatexConsole` to the pre-baked path** (drop their runtime `SchemaCompiler.Compile` calls in favour of the generated `BuildParser(visitor)`). `lalr-tui` stays runtime-build because its purpose is editing arbitrary YAML live.
   - **Slice 6 (optional) — pre-bake the lexer too.** Today the lexer half is still runtime-built via `SchemaCompiler.BuildLexer` + `IRxParser`. To trim those too, link `IRxParser` + the IRx AST types into the netstandard2.0 generator and emit `LexRule[]` arrays with `IRx` constructor expressions. Bigger lift; defer until users actually feel the SchemaCompiler size.
