...
1. **Phase 5 (compiler-compiler) — slice 6 (optional).** Slices 1–5 landed (`ParserTableBuilder` extracted, linked into the netstandard2.0 generator, `TablesEmitter` produces `<Name>.Tables.g.cs` with a pre-baked `Definition` + `ParseTable` + `BuildParser()` per YAML grammar; `VisitorEmitter` adds a `BuildParser<T>(IVisitor<T>)` overload that splices visitor rewriters into the pre-baked grammar and reuses the pre-baked table; all in-tree consumers — `Bootstrap.Stage1`, `Examples.Calculator` / `.Json` / `.Latex` / `.LatexConsole` — route through `BuildParser(visitor)` so `ParserTableBuilder`'s LR0/LR1 helpers are unreachable in their AOT trim graphs; `LALR0004` surfaces S/R + R/R conflicts as Roslyn diagnostics at build time). `lalr-tui` stays runtime-build because its purpose is editing arbitrary YAML live. Remaining work:
   - **Slice 6 (optional) — pre-bake the lexer too.** Today the lexer half is still runtime-built via `Build(visitor).Lexer` → `SchemaCompiler.BuildLexer` → `IRxParser`. To trim those too, link `IRxParser` + the IRx AST types into the netstandard2.0 generator and emit `LexRule[]` arrays with `IRx` constructor expressions, plus a `BuildLexer()` factory. Bigger lift; defer until users actually feel the SchemaCompiler size.
