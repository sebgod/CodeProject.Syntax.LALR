...
public class Debug
{
    ...
    public void DumpLR0Item(LR0Item item)
    {
        ...
        for (; ; )
        {
            ...
            var nToken = _parser.Productions[item.Production].Right[nPosition];
    ...
    {
        for (var nStateID = 0; nStateID < _parser.LALRPropogations.Count; nStateID++)
        {
    ...
    public string TokenInfo(Item item) => TokenInfo(item, false);
        ...
        if (_parser.NonTerminals.Contains(item.ID))
        {
            switch (item.ContentType)
            {
                case ContentType.Scalar:
                    var stringified = item.Content is IEnumerable<object> enumerable
    ...
    private static string FormatItem(Item item) => EscapeString(item.ToString());

    private static string EscapeString(string item) => item switch
    {
        " " => "{ }",
        "," => "{,}",
        _ => item.Replace("\n", @"\n").Replace("\t", @"\t"),
